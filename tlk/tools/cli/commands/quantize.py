...
def quantize(model_dir, dataset_dir, inc_config, batch_size, accuracy_criterion, timeout, max_trials, output_dir):
    ...
    # Get the model name from the directory path, assuming our TLK models are exported like <model name>/n
    ...
    try:
        ...
        sys.exit("An error occurred while getting the model: {}\nNote that the model directory is expected to contain "
                 "a model that was exported by TLK where the directory structure has the <model name>/n/saved_model.pb "
    ...
    try:
        ...
        model.post_training_quantization(model_dir, quantized_output_dir, inc_config)
