...
|-----|------|---------|-------------|
| `metadata.name` | string | `distributed-pytorchjob` | Name of the PyTorch job that is deployed to kubernetes. This also ends up being the prefix for the name of the worker pods. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `secret.name` | string | `hf-token-secret` | Name of the Kubernetes secret object. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `image.name` | string | `amr-registry.caas.intel.com/aiops/mlops-ci` | Name of the image to use for the PyTorch job. The container should include the fine tuning script and all the dependencies required to run the job. |
| `image.tag` | string | `dmsuehir-chatbot-finetune-oneccl` | The image tag for the container that will be used to run the PyTorch job. The container should include the fine tuning script and all the dependencies required to run the job. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `distributed.train.dataFile` | string | `/workspace/data/<TBD>.json` | The Llama2 formatted data file to use. |
...
| `distributed.train.outputDir` | string | `/tmp/output/bf16_saved_model` | The output directory where the model predictions and checkpoints will be written. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `distributed.quantize.peftModelDir` | string | `/tmp/output/bf16_saved_model` | When quantization is enabled, the path the Peft model to load. If Peft was not used during training, leave this field blank and the `distributed.modelNameOrPath` will be loaded for quanitization. |
| `distributed.quantize.outputDir` | string | `/tmp/output/bf16_quantized_model` | Location to save the quantized model, when quantization is enabled. |
...
| envVars.transformersCache | string | `/tmp/output/transformers_cache` | Location for the Transformers cache (using the TRANSFORMERS_CACHE environment variable). |
| envVars.hfDatasetsCache | string | `/tmp/output/dataset_cache` | Path to a directory used to cache Hugging Face datasets using the HF_DATASETS_CACHE environment variable. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| resources.cpuRequest | string | `200` | The amount of CPU resources requested for each worker, where 1 CPU unit is equivalent to 1 pysical CPU core or 1 virtual core. For more information see the [Kubernetes documentation on CPU resource units](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#meaning-of-cpu). |
| resources.cpuLimit | string | `200` | The maximum amount of CPU resources for each worker, where 1 CPU unit is equivalent to 1 pysical CPU core or 1 virtual core. For more information see the [Kubernetes documentation on CPU resource units](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#meaning-of-cpu). |
| resources.memoryRequest | string | `226Gi` | The amount of memory resources requested for each worker. For more information see the [Kubernetes documentation on memory resource units](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#meaning-of-memory). |
| resources.nodeSelectorLabel | string | `team` | Specify a label for the type of node that will be used for the PyTorch job workers. |
| resources.nodeSelectorValue | string | `dell-poc` | Specify the value for the label for the type of node that will be used for the PyTorch job workers. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| storage.pvcName | string | `clm-pvc` | Name of the persistent volume claim. |
...
| storage.pvcMountPath | string | `/tmp/output` | The location where the persistent volume claim will be mounted in the worker pods. |
