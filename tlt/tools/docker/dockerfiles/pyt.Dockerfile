...
RUN apt-get update && \
    ...
    pip install --no-cache-dir tlt-${TLT_VERSION}-py2.py3-none-any.whl[pytorch] && \
    rm tlt-${TLT_VERSION}-py2.py3-none-any.whl
...
CMD ["sh", "-c", \
     "TORCH_HOME=${OUTPUT_DIR}/torch_cache tlt train --framework pytorch --model-name ${MODEL_NAME} --output-dir ${OUTPUT_DIR} --dataset-dir ${DATASET_DIR} --epochs ${EPOCHS} --dataset-name \"${DATASET_NAME}\" "]
