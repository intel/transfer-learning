...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `image.name` | string | `amr-registry.caas.intel.com/aiops/mlops` <!-- TODO: Replace with the public image when it's available --> | Name of the image to use for the PyTorch job. The container should include the fine tuning script and all the dependencies required to run the job. |
| `image.tag` | string | `torch-2.0.1-huggingface-multinode-python-3.9` <!-- TODO: Replace with the public tag when it's available --> | The image tag for the container that will be used to run the PyTorch job. The container should include the fine tuning script and all the dependencies required to run the job. |
